/**
 * Program Detail API - HOVCS 2.0 Conservative Core 적용
 */
...
export const PATCH = withErrorHandler(async (
  ...
  await logDataUpdate('Program', program.id, user?.userId, user?.name, {
...
export const DELETE = withErrorHandler(async (
  ...
  // 감사 로그 기록
  const user = getUserFromRequest(request);
  await logDataDelete('Program', id, user?.userId, user?.name);
